                ...
                <div class="events">
                  ...
                  >
                    ...
                    <div class="event-card">
                      ...
                      <div v-if="event.snapshot" class="event-attrs">
                        <span
                          v-for="(val, key) in event.snapshot"
                          :key="String(key)"
                          :class="{ changed: val !== getPrevValue(event, key as SnapshotKey) }"
                        >
                          {{ key }}: {{ val }}
                        </span>
                      </div>
...
import type { CharacterAttributes, HistoryEntry } from '@/core/types/game';

type SnapshotKey = keyof CharacterAttributes;
type TimelineEvent = {
  id: string;
  nodeId: string;
  title: string;
  isChoice: boolean;
  choiceText?: string;
  snapshot: Partial<CharacterAttributes>;
  index: number;
};
...
const groupedHistory = computed(() => {
  ...
  let currentGroup: { time: string; events: TimelineEvent[] } | null = null;
...
function canJumpTo(event: TimelineEvent): boolean {
  const currentIndex = gameStore.history.findIndex(
    entry => entry.nodeId === gameStore.gameState.currentNode
  );
  return currentIndex === -1 || event.index < currentIndex;
}

function getPrevValue(event: TimelineEvent, key: SnapshotKey): number {
  const prevEvent: HistoryEntry | undefined = gameStore.history[event.index - 1];
  return prevEvent?.attributesSnapshot?.[key] ?? 50;
}

function jumpTo(event: TimelineEvent) {
