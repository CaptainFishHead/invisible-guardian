# 开发《隐形守护者》风格互动叙事游戏所需的完整库支持清单，按功能模块分类：
...
import { NodeType, EffectType } from '../types/story';

export class StoryEngine {
  // 私有状态
  private chapter = ref<Chapter | null>(null);
  private currentNodeId = ref<string>('');
  ...
  // 只读公开状态
  public readonly currentChapter = readonly(this.chapter);
  public readonly currentNodeId = readonly(this.currentNodeId);

  // 计算属性
  public currentNode = computed<StoryNode | null>(() => {
    if (!this.chapter.value) return null;
    return this.chapter.value.nodes.get(this.currentNodeId.value) || null;
  });
  ...
  loadChapter(chapterData: Chapter, startNodeId?: string): boolean {
    try {
      ...
      this.currentNodeId.value = startId;
  ...
  jumpTo(nodeId: string, recordHistory = true): boolean {
    ...
    this.currentNodeId.value = nodeId;
    this.executeNodeEnter();
  ...
  makeChoice(choice: Choice): boolean {
    // 记录选择
    this.choiceHistory.value.set(this.currentNodeId.value, choice.id);
  ...
  goBack(): boolean {
    ...
    // 不记录历史地跳转
    this.currentNodeId.value = prevNodeId;
    this.executeNodeEnter();
  ...
  jumpToHistory(index: number): boolean {
    ...
    this.currentNodeId.value = targetId;
    this.executeNodeEnter();
  ...
  getStoryTree(): any {
    ...
    return {
      ...
      current: this.currentNodeId.value
    };
  }
...
export default defineConfig({
  ...
  build: {
    ...
    rollupOptions: {
      output: {
        manualChunks: {
          'chapter1': ['./src/data/chapters/chapter1.ts'],
          'chapter2': ['./src/data/chapters/chapter2.ts']
        }
      }
    }
  },
...
<html lang="zh-CN">
  <head>
    ...
    <title>隐形守护者 - The Invisible Guardian</title>
    <style>
      body {
        margin: 0;
        overflow: hidden;
